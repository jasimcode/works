...
paintBrush = pygame.image.load(
    "E:\code works\Python-Drawing-Program-master\Paintbrush.png")
currentVersion = 1.1
...
def save(cols, rows, show, grid, path):
    ...
    for pixel in grid:
        for p in pixel:
            wr = str(p.color[0]) + ',' + str(p.color[1]) + ',' + str(
                p.color[2])
            file.write(wr + '\n')
            ...
            try:
                ...
                if pos[1] >= grid.height:
                    if pos[0] >= tools.startx and pos[
                            0] <= tools.startx + tools.width and pos[
                                1] >= tools.starty and pos[
                                    1] < +tools.starty + tools.height:
                        replace = False
                        doFill = False
                        tools.drawGrid()
                        buttons = ['D', 'E', 'F', 'R', 'C']
                    ...
                    elif pos[0] >= saveMenu.startx and pos[
                            0] <= saveMenu.startx + saveMenu.width and pos[
                                1] >= saveMenu.starty and pos[
                                    1] <= saveMenu.starty + saveMenu.height:
